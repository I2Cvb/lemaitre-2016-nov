"""
This pipeline aimed at creating balanced set using condensed nearest neighbor
approach.
"""
...
path_store = '/data/prostate/balanced/lemaitre-2016-nov/cnn'
...
for idx_pat, (data_pat, label_pat) in enumerate(zip(data, label)):
    ...
    # Balanced the set
    cnn = CondensedNearestNeighbour(verbose=True)

    # Under-sample the data
    data_b, label_b = cnn.fit_transform(data[idx_pat], label[idx_pat])
