"""
This pipeline aimed at creating balanced set using random one-sided selection
approach.
"""
...
path_store = '/data/prostate/balanced/lemaitre-2016-nov/one-sided-sel'
...
for idx_pat, (data_pat, label_pat) in enumerate(zip(data, label)):
    ...
    # Balanced the set
    oss = OneSidedSelection(verbose=True)

    # Under-sample the data
    data_b, label_b = oss.fit_transform(data[idx_pat], label[idx_pat])
