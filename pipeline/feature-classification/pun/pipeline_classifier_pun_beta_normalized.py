"""
This pipeline is used to classify Pun parameters.
"""
...
path_pun = '/data/prostate/pre-processing/lemaitre-2016-nov/pun-features-normalized'
...
for idx_pat in range(len(id_patient_list)):
    ...
    filename_pun = (id_patient_list[idx_pat].lower().replace(' ', '_') +
                      '_pun.npy')

    # Concatenate the training data
    # Select the parameter beta
    data.append(np.load(os.path.join(path_pun, filename_pun))[:, 2])
...
path_store = '/data/prostate/results/lemaitre-2016-nov/pun-beta-normalized'
