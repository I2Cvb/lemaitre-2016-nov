...
dict_size = [100, 200, 300, 400, 500, 600, 700 , 800, 1000]
sp_level = [2, 4, 8, 16]
...
for c in config:
    results_dict = []
    for d_sz in dict_size:
        results_sp = []
        for sp in sp_level:

            print 'The following configuration will be used: {}'.format(c)
            print 'KSVD parameters: {} dictionary size'
            ' - {} sparsity level {}'.format(d_sz, sp)
            ...
            for idx_lopo_cv in range(len(id_patient_list)):
                ...
                # Learn the dicitionary
                D, gamma_training = KSVD(training_data, d_sz, sp, 1000)

                # Project the testing data
                gamma_testing = KSVD_Encode(testing_data, D, sp)

                # Perform the classification for the current cv and the
                # given configuration
                result_cv.append(Classify(gamma_training, training_label,
                                          gamma_testing, testing_label,
                                          **c))

            results_sp.append(result_cv)

        results_dict.append(results_sp)

    # Concatenate the results per configuration
    result_config.append(results_dict)

# Save the information
path_store = '/data/prostate/results/lemaitre-2016-nov/sparse-ksvd'
